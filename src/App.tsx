...
import { Player, BiometricResonance, LobbyState, GlobalJackpotState, Challenge, PaymentRequirement } from './types';
...
export default function App() {
  ...
  const handleSelectNumber = async (num: number) => {
    ...
    try {
      const res = await fetch('/api/v1/interlink/execute', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleSignAndPay = async () => {
    ...
    try {
      ...
      const endpoint = isLobbyJoin ? '/api/lobbies/join' : '/api/v1/interlink/execute';
